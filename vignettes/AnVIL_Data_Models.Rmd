...
```{r}
anvil_import_set(tables$sample_set, "sample_set", model, overwrite=TRUE)
```

We mgiht find it convenient to have a set containing all samples, so we can add that to the sample_set table.

```{r}
all <- create_set_all(tables$sample, "sample")
anvil_import_set(all, "sample_set", model, overwrite=TRUE)
avtable("sample_set")
```
